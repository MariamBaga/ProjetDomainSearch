...
use Illuminate\Support\Facades\Log; // Correctement importé
...
class DomainController extends Controller
{
    ...
    public function indexTransfer($domainId)
    {
        ...
        // Loguer l'email de l'utilisateur
        Log::info('User trying to transfer domain', ['email' => $userEmail, 'domainId' => $domainId]);

        // Récupérer le domaine par ID
        $domain = Domain::where('id', $domainId)
            ->where('user_email', $userEmail) // Vérifier avec l'email de l'utilisateur connecté
            ->first();
    ...
    public function indexRenew($domainId)
    {
        ...
        // Loguer l'email de l'utilisateur
        Log::info('User trying to renew domain', ['email' => $userEmail, 'domainId' => $domainId]);
